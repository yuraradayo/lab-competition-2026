...
namespace orienteering {
    ...
    // ============================================================
    // 修復：選択数を MIN_CONTROLS〜MAX_CONTROLS の範囲に戻す
    // ============================================================
    void repair_control_count(Chromosome& chromosome, int N, RNG& rng) {
        int count = 0;
        for (int i = 0; i < N; ++i) {
            if (chromosome[i] == 1) {
                ++count;
        ...
        // 選択数が少なすぎる場合、未選択候補からランダムに追加
        while (count < MIN_CONTROLS) {
            ...
            std::uniform_int_distribution<int> dist(
                0, static_cast<int>(unselected.size()) - 1);
            chromosome[unselected[dist(rng)]] = 1;
            ++count;
        }

        // 選択数が多すぎる場合、選択候補からランダムに削除
        while (count > MAX_CONTROLS) {
            ...
            chromosome[selected[dist(rng)]] = 0;
            --count;
        }
    }
        ...
        RNG& rng)
    {
        ...
        for (int gen = 1; gen <= N_GEN; ++gen) {
            ...
            while (static_cast<int>(next_pop.size()) < POP_SIZE) {
                ...
                // 交叉・突然変異で制約範囲外になった個体を修復
                repair_control_count(children.first, N, rng);
                repair_control_count(children.second, N, rng);
